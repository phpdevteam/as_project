   ...
   function escapeExcelValue($specialString)
	...
	function replaceSpecialChar($specialString)
	{
		if($specialString != "")
		{
			...
			$special_str = htmlentities($special_str, ENT_QUOTES);
			$special_str = mysql_real_escape_string($special_str);
	...
	function replaceSpecialCharBack($specialString)
	{
		if($specialString != "")
		{
			$special_str = $specialString;
			$special_str = trim($special_str);
			$special_str = html_entity_decode($special_str);
			/*$special_str = str_replace('&#92;', "\\\\", $special_str);
			$special_str = str_replace('&#39;', "'", $special_str);
			$special_str = str_replace('&quot;', '"', $special_str);*/
		}
...
{ 
	global $tbname;
	//Q/2013/0043
	 $mstr1 = "SELECT REPLACE(REPLACE(_quotationno,'CGQ-',''),'R','') as MCount FROM ".$tbname."_salequotations sq Where _quotationno = '". $rno ."' ";
...
function getMaxOrder($sqid)
{
	global $tbname;
	//Q/2013/0043
	 $mstr1 = "SELECT max(_Order) as MCount FROM ".$tbname."_salequotationitems sq Where _QutotationId = '". $sqid ."' and _status = 1 ";	
...
function generateInvoiceNo($company)
{
	global $tbname;
	 $mstr1 = "SELECT Max(_invoiceno) as MCount FROM ".$tbname."_invoices sq WHERE _createdbycompany = '".$company."'  ";	
	$mrst = mysql_query($mstr1);
	$mcount = 0;
	if(mysql_num_rows($mrst)>0)
	{
...
{
	global $tbname;
	 $mstr1 = "SELECT _orderno as MCount FROM ".$tbname."_receipts sq ORDER BY _createddate DESC LIMIT 1 ";	
	$mrst = mysql_query($mstr1);
	if(mysql_num_rows($mrst)>0)
	{
		$mrow = mysql_fetch_assoc($mrst);
	...
	$mcount = str_pad($mcount, 5, "0", STR_PAD_LEFT);
	return "RC".date('Y').generateCode(3).$mcount;
}
