<?
   function escapeExcelValue($specialString)
	...
	function datepickerToMySQLDateTime($date)
	{
		if(trim($date)!="" )
		{ 
			$date = DateTime::createFromFormat("d/m/Y H:i", trim($date));		
...
{
	global $tbname;
	$mstr1 = "SELECT max(_customerid) as MCount FROM ".$tbname."_client ORDER BY _createddate DESC LIMIT 1";	
...
function generateTrainerNo()
{
	global $tbname;
	$mstr1 = "SELECT max(_TrainerRefNo) as MCount FROM ".$tbname."_trainers ORDER BY _CreatedDateTime DESC LIMIT 1";	
	...
	if(mysql_num_rows($mrst)>0)
	{
		...
		$mcount = $mrow['MCount'];
		$mlength = strlen($mcount);
		$mcount = substr($mcount,($mlength-4));
	}
	if($mcount<=0)
	{
		$mcount = 1;
	...
	$mcount = str_pad($mcount, 4, "0", STR_PAD_LEFT);
	return "TR-".date("Y")."-".$mcount;
}


function generatetranVenueNo()
{
	global $tbname;
	$mstr1 = "SELECT max(_VenueRefNo) as MCount FROM ".$tbname."_venueowners ORDER BY _CreatedDateTime DESC LIMIT 1";	
	...
	$mcount = str_pad($mcount, 4, "0", STR_PAD_LEFT);
//	return "TS-".date("Y")."-".$mcount;

	return "VO-".date("Y"). "-".date("m")."-".$mcount;
}
...
function generateTrainervenuerefNo()
{
	global $tbname;
	$mstr1 = "SELECT max(_TraningVenuNo) as MCount FROM ".$tbname."_venues ORDER BY _CreatedDateTime DESC LIMIT 1";	
...
//	return "TS-".date("Y")."-".$mcount;
	...
	return "TS-".date("Y"). "-".date("m")."-".$mcount;
}
...
{ 
	global $tbname;
	//Q/2013/0043
	 $mstr1 = "SELECT max(Cast(REPLACE(_quotationno,'CGSQ-','') as signed)) as MCount FROM ".$tbname."_salequotations sq Where _quotationno not like '%R%' ";
	$mrst = mysql_query($mstr1);
	$mcount = 0;
	if(mysql_num_rows($mrst)>0)
	{
...
function generateProgramNo()
{
	global $tbname;
	$mstr1 = "SELECT max(_ProgramRefID) as MCount FROM ".$tbname."_trainingprogram ORDER BY _CreatedDateTime DESC LIMIT 1";	
...
{ 
	global $tbname;
	//Q/2013/0043
	 $mstr1 = "SELECT max(Cast(REPLACE(_srno,'CGSR-','') as signed)) as MCount FROM ".$tbname."_servicereports sq ";
	$mrst = mysql_query($mstr1);
	$mcount = 0;
	if(mysql_num_rows($mrst)>0)
	{
...
function getMaxOrder($sqid,$tblname,$fieldname,$searchfield)
{
	global $tbname;
	//Q/2013/0043
	  $mstr1 = "SELECT max(". $fieldname.") as MCount FROM ".$tbname. $tblname ." sq 
...
{
	global $tbname;
	 $mstr1 = "SELECT max(Cast(REPLACE(_purchaseorderno,'CGPO-','') as signed)) as MCount FROM ".$tbname."_purchaseorders sq ORDER BY _createddate DESC LIMIT 1 ";	
	$mrst = mysql_query($mstr1);
...
{
	global $tbname;
	$mstr1 = "SELECT max(Cast(REPLACE(_deliveryorderno,'CGDO-','') as signed)) as MCount FROM ".$tbname."_deliveryorders sq ORDER BY _createddate DESC LIMIT 1 ";	
	$mrst = mysql_query($mstr1);
...
{
	global $tbname;
	 $mstr1 = "SELECT max(Cast(REPLACE(_invoiceno,'CGI-','') as signed)) as MCount FROM ".$tbname."_invoices sq ORDER BY _createddate DESC LIMIT 1 ";	
	$mrst = mysql_query($mstr1);
	$mcount = 0;
	if(mysql_num_rows($mrst)>0)
	{
...
function generatePInvoiceNo()
{
	global $tbname;
	 $mstr1 = "SELECT max(Cast(REPLACE(_InvoiceNo,'CGPI-','') as signed)) as MCount 
	 FROM ".$tbname."_profomas sq ";	
	$mrst = mysql_query($mstr1);
	$mcount = 0;
	if(mysql_num_rows($mrst)>0)
	{
...
{
	global $tbname;
	 $mstr1 = "SELECT max(Cast(REPLACE(_orderno,'CGR-','') as signed)) as MCount FROM ".$tbname."_receipts sq ORDER BY _createddate DESC LIMIT 1 ";	
	$mrst = mysql_query($mstr1);
