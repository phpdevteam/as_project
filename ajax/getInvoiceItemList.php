...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="grid" id="salesItems">
    ...
    <?
            ...
            $sql = "SELECT  si._unitprice,si._qty,si._id, si._description, st._typename as _salestypename, p._productname as _pdescription FROM ".$tbname."_invoiceitems si INNER JOIN ".$tbname."_products p ON p._id = si._productid 
    ...
    <tr id="trSalesItem_<?=$si?>">
        ...
        if($sirow['_qty']!="" && $sirow['_unitprice']!="")
         ...
         $str2 = "SELECT _discount FROM ".$tbname."_invoicediscount WHERE (_itemid = '".$sirow['_id']."') ";
    ...
    <?		$si++;			
            ...
            $sql = "SELECT  si._unitprice,si._qty,si._id, si._description, st._typename as _salestypename, p._productname as _pdescription FROM ".$tbname."_invoiceitemsdraft si  INNER JOIN ".$tbname."_products p ON p._id = si._productid 
    ...
    <tr id="trSalesItem_<?=$si?>">
        ...
        if($sirow['_qty']!="" && $sirow['_unitprice']!="")
            $totalSum = $sirow['_qty']*$sirow['_unitprice'];
         $str2 = "SELECT _discount FROM ".$tbname."_invoicediscountdraft WHERE (_itemid = '".$sirow['_id']."') ";
              $rst2 = mysql_query($str2);
             while($rs2 = mysql_fetch_assoc($rst2)){ 
                  $totalSum = (float)$totalSum - (float)$rs2['_discount'];						  							
             }
