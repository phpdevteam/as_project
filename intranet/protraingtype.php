<?php
 ...
 $Operations = GetAccessRights(148);
...
if(count($Operations)== 0)
{
 echo "<script language='javascript'>history.back();</script>";
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_trainingtype WHERE _ID = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$ProgramTypeName = $rs["_TypeName"];		
			$status = $rs["_Status"];			
			$btnSubmit = "Update";
		}
	}
	...
	if($_GET['error'] == 1)
	{
		$ProgramTypeName = $_GET["ProgramTypeName"];
	}
?>
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.ProgramTypeName.value == 0)
                errormsg += "Please fill in 'Program Type Name'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "protraingtype_action.php";
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings &gt; Programs &gt; Programs Type </b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							...
							<form name="FormName" action="protraingtype_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?=$id?>" />
							<input type="hidden" name="e_action" value="<?=($e_action == "" ? "Add" : "Edit")?>" />
							<input type="hidden" name="PageNo" value="<?=$_GET['PageNo']?>" />
							<table cellpadding="1" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b>Programs Type </b></td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
									<td width="120">Programs Type </td>
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="ProgramTypeName" value="<?=$ProgramTypeName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
                                <? if ($e_action=="edit") { ?>								
								<tr>
									<td nowrap>&nbsp;Status&nbsp;</td>
							...
							<?php
								...
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Record [".$ProgramTypeName."] is existed in the system. Please enter another Record.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT t._ID, t._TypeName,t._CreatedDateTime,ss._StatusName	 FROM ".$tbname."_trainingtype t

							INNER JOIN ".$tbname."_systemstatus ss ON ss._id = t._Status";
							
							
							if ($Keywords != "") $str1 = $str1 . " AND _TypeName LIKE '%".replaceSpecialChar($Keywords)."%' ";
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY ss._StatusName ,t._TypeName ASC LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Programs Type List</b></td>
								</tr>
                               	<form name="search_form" action="protraingtype.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="text" tabindex="" title="Programs Type" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
                                <tr>
                                  <td></td>
                        ...
                        elseif (trim($sortArrange) == "ASC")
							...
							{
								?>
                                    <a href="protraingtype.php" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="1" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader" width="40" align="center">No.</td>
												<td class="gridheader" width="250" >&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_TypeName&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Program Type</a></td>
												...
												<td class="gridheader datecolumn" width="200"  align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_CreatedDateTime&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Submitted Date</a></td>
                                                
                                                <td class="gridheader" width="150"  align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_StatusName&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">System Status</a></td>
												
                                               <td class="gridheader" width="100" align="center">Edit</td>
												...
												while($rs = mysql_fetch_assoc($result))
												{
													...
													<tr >
													  ...
													  <input name="CustCheckbox<?php echo $i; ?>" type="checkbox"   tabindex="" value="<?php echo $rs["_ID"]; ?>" onclick="setActivities(this.form.CustCheckbox<?php echo $i; ?>, <?php echo $i; ?>,'<?=$Rowcolor?>');" />
                                                        ...
                                                        <!--<td id="Row4ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_Fullname"]; ?>&nbsp;</td>-->
														
														 <td id="Row5ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_CreatedDateTime"]!=""?date(DEFAULT_DATEFORMAT,strtotime(replaceSpecialCharBack($rs["_CreatedDateTime"]))):"" ?>&nbsp;</td>
									...
									if(in_array("Edit",$Operations))
							{
								?>
                                                        ...
                                                        <a href="?PageNo=<?=$PageNo?>&amp;id=<?=$rs["_ID"]?>&amp;e_action=edit" class="TitleLink">Edit</a>&nbsp;</td>
