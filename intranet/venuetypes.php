<?php
 ...
 $Operations = GetAccessRights(206);
	...
	if($id != "" && $e_action == 'edit')
	{
		...
		$str = "SELECT * FROM ".$tbname."_venuetype WHERE _ID = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$VenueTypeName = $rs["_VenueTypeName"];	
			$bookwithoutprog = $rs["_BookWithoutProg"];
			$status = $rs["_Status"];		
			$btnSubmit = "Update";
		}
	}
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.VenueTypeName.value == 0)
                errormsg += "Please fill in 'Add Venue Type'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "venuetypes_action.php?e_action=curdelete";
					...
					<tr>
						...
						-->
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings > Venue Type </b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							<form name="FormName" action="venuetypes_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								...
								<tr>
									<td width="120" valign="middle">Add Venue Type</td>
									<td width="10" valign="middle">&nbsp;:&nbsp;</td>
									<td valign="middle"><input type="text" tabindex="" name="VenueTypeName" value="<?=$VenueTypeName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
                                            <td width="140" valign="middle">Book Without Program </td>
                                            <td valign="middle">&nbsp;:&nbsp;</td>
                                            <td valign="middle">
                                            <input type="checkbox"   tabindex="" id="bookwithoutprog" name="bookwithoutprog" value="1" <?php if( trim($bookwithoutprog) == '1') echo 'checked';?>/>                                                                                             
                                            </td>
							...
							<?php
								if ($_GET["done"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>VenueType has been added successfully.<br></font></div>";
								}
								if ($_GET["done"] == 2)
								{
									echo "<div align='left'><font color='#FF0000'>VenueType has been edited successfully.<br></font></div>";
								}
								if ($_GET["done"] == 3)
								{
									echo "<div align='left'><font color='#FF0000'>VenueType has been deleted successfully.<br></font></div>";
								}
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>VenueType [".$VenueTypeName."] is existed in the system. Please enter another Type.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT t._ID,IF(t._Status = 1, 'Live', 'Archived') as _Status,_VenueTypeName,_BookWithoutProg,_CreatedDate,usr._Fullname as _subby 
							FROM ".$tbname."_venuetype  t
							left join  ".$tbname."_user usr on (t._SubmittedBy = usr._ID)	 
							
							WHERE t._Status <> 0 ";		
				
							
							if ($Keywords != "") $str1 = $str1 . " AND _VenueTypeName LIKE '%".replaceSpecialChar($Keywords)."%' ";

							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . "ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
                                $str2 = $str1 . "ORDER BY t._Status,_VenueTypeName LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="2" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Venue Type List</b></td>
								</tr>
									<form name="search_form" action="venuetypes.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="hidden" name="type" value="<?=$type?>" /><input type="hidden" name="id" value="<?=$id?>" /><input type="text" tabindex="" title="Type" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
							...
							{
								?>
                                    	<a href="venuetypes.php?type=<?=$type?>" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="2" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader" width="30%">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_VenueTypeName&amp;sortArrange=<?=$sortArrange?>" class="link1">VenueType Name</a></td>

												<td class="gridheader"  width="10%" align="center" >&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_BookWithoutProg&amp;sortArrange=<?=$sortArrange?>" class="link1">Book Without Program</a></td>

                                                  <td class="gridheader datecolumn"  width="10%" align="center">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_subby&amp;sortArrange=<?=$sortArrange?>" class="link1">Submitted By</a></td>
												
												<td class="gridheader datecolumn"  width="10%" align="center">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_CreatedDate&amp;sortArrange=<?=$sortArrange?>" class="link1">Submitted Date</a></td>
                                              
                                                <td class="gridheader"   width="10%" align="center">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_Status&amp;sortArrange=<?=$sortArrange?>" class="link1">System Status</a></td>
											...
											if($RecordCount!="")
											{
												...
												while($rs = mysql_fetch_assoc($result))
												{
													...
													<tr >
														...
														<?php echo $rs["_VenueTypeName"]?>&nbsp;
														</td> 

														<td id="Row4ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="left" >&nbsp;
														<?php  if($rs["_BookWithoutProg"]==1){echo "Yes";} else {echo "No";}?>&nbsp;
														</td> 
                                                        <td id="Row5ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_subby"]; ?>&nbsp;</td>
														
                                                        <td id="Row6ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $date;?>&nbsp;</td>
