<?php
 ...
 include("fckeditor/fckeditor.php");
	...
	$id = trim($_REQUEST['id']);
	$Country = trim($_REQUEST['CountryName']);	
	...
	if($e_action == "Add")
	{
		$str = "SELECT * FROM ".$tbname."_countries WHERE _countryname = '". replaceSpecialChar($Country) ."' and _status = 1 ";
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			include('../dbclose.php');
			echo "<script language='JavaScript'>window.location = 'countries.php?PageNo=".$PageNo."&error=1&CountryName=".$Country."'</script>";
			exit();
		}
		
		$str = "INSERT INTO ".$tbname."_countries (_countryname, _subdate, _subby, _status) VALUES(";
		if($Country != "")
		...
		if ($Country != "")
		{
			$strSQL = $strSQL . "'" . replaceSpecialChar($Country) . "' ";
		}
		else
		{
			$strSQL = $strSQL . "null ";
		...
		include('../dbclose.php');
		echo "<script language='JavaScript'>window.location = 'countries.php?PageNo=".$PageNo."&done=1'</script>";
		exit();
	}
	else if($e_action == "Edit")
	{
		
		$str = "UPDATE ".$tbname."_countries SET ";
		if($Country != "")
			$str = $str . "_countryname = '" . replaceSpecialChar($Country) . "', ";
		else
			$str = $str . "_countryname = null, ";					
		...
		include('../dbclose.php');
		echo "<script language='JavaScript'>window.location = 'countries.php?PageNo=".$PageNo."&done=2'</script>";
		exit();
	}
	else if($e_action == "curdelete")
	{
		...
		$str = "SELECT DISTINCT _countryname FROM ".$tbname."_countries WHERE (" . $emailString . ") ";
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			...
			{	
				...
				$strSQL = $strSQL . "'Delete Country', ";
				if ($rs["_countryname"] != "")
				{
					$strSQL = $strSQL . "'" . replaceSpecialChar($rs["_countryname"]) . "' ";
				}
				else
				{
					$strSQL = $strSQL . "null ";
		...
		$str = "UPDATE ".$tbname."_countries SET _status = '2' ";
		$str = $str . " WHERE (" . $emailString . ") ";
		mysql_query($str);			
		include('../dbclose.php');
		echo "<script language='JavaScript'>window.location = 'countries.php?done=3'</script>";
		exit();
	}
?>