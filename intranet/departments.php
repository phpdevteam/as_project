<?php
 ...
 include("fckeditor/fckeditor.php");
	...
	$Operations = GetAccessRights(195);
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_departments WHERE _id = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$DepartmentName = $rs["_departmentname"];		
			$status = $rs["_status"];			
			$btnSubmit = "Update";
		}
	}
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.DepartmentName.value == 0)
                errormsg += "Please fill in 'Department'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "departments_action.php";
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings - System - Departments</b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							<form name="FormName" action="departments_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?=$id?>" />
							<input type="hidden" name="e_action" value="<?=($e_action == "" ? "Add" : "Edit")?>" />
							<input type="hidden" name="PageNo" value="<?=$_GET['PageNo']?>" />
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b><?=($e_action == "" ? "Add" : "Edit")?> Departments</b></td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
									<td width="120" valign="middle">&nbsp;Department</td>
									<td width="10"  valign="middle">&nbsp;:&nbsp;</td>
									<td  valign="middle"><input type="text" tabindex="" name="DepartmentName" value="<?=$DepartmentName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
							...
							<?php
								...
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Record [".$DepartmentName."] is existed in the system. Please enter another Record.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT c._id, c._departmentname,usr._Fullname as _subby, c._subdate,  c._status, ss._statusname 
							FROM ".$tbname."_departments c 
							INNER JOIN ".$tbname."_systemstatus ss ON ss._id = c._status
							INNER JOIN ".$tbname."_user usr ON c._subby = usr._id
							Where c._id > 0 ";		
//							INNER JOIN ".$tbname."_user u ON u._ID = c._subby 					
							if ($Keywords != "") $str1 = $str1 . " AND c._departmentname LIKE '%".replaceSpecialChar($Keywords)."%' ";
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY c._status, c._departmentname ASC LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Department List</b></td>
								</tr>
                               	<form name="search_form" action="departments.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="text" tabindex="" title="Department Name" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
                                <tr>
                                  <td></td>
                        ...
                        elseif (trim($sortArrange) == "ASC")
							...
							{
								?>
                                    <a href="departments.php" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="0" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_departmentname&amp;sortArrange=<?=$sortArrange?>" class="link1">Departments</a></td>
												
												  <td class="gridheader datecolumn" align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_subby&amp;sortArrange=<?=$sortArrange?>" class="link1">Submitted By</a></td>
							...
							if(in_array("Edit",$Operations))
							{
								?>
                                                        ...
                                                        <a href="departments.php?e_action=edit&id=<?=$rs["_id"]?>" class="TitleLink">Edit</a>&nbsp;
