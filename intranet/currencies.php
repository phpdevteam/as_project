...
$Operations = GetAccessRights(151);
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_currencies WHERE _id = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$currencyshortname = $rs["_currencyshortname"];		
			$currencysymbol = $rs["_sign"];	
			$status = $rs["_status"];			
			$btnSubmit = "Update";
		}
	}
	...
	if($_GET['error'] == 1)
	{
		$currencyshortname = $_GET["currencyshortname"];
	}
?>
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.currencyshortname.value == 0)
                errormsg += "Please fill in 'Currency'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "currencies_action.php";
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings - System - Currencies</b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							<form name="FormName" action="currencies_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?=$id?>" />
							<input type="hidden" name="e_action" value="<?=($e_action == "" ? "Add" : "Edit")?>" />
							<input type="hidden" name="PageNo" value="<?=$_GET['PageNo']?>" />
							<table cellpadding="1" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b><?=($e_action == "" ? "Add" : "Edit")?> Currency</b></td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
									<td width="120">Currency </td>
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="currencyshortname" value="<?=$currencyshortname?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
                                <tr>
									<td width="120">Short Code</td>
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="currencysymbol" value="<?=$currencysymbol?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
                                <? if ($e_action=="edit") { ?>								
								<tr>
									<td nowrap>&nbsp;Status&nbsp;</td>
									...
									?>		
									<td>Live<input type="radio"   tabindex="" <?php echo $sel_status_y; ?> name="status" value="1" id="RadioGroup1_0" class="radio" />
									Archive<input type="radio"   tabindex="" <?php echo $sel_status_n; ?> name="status" value="2" id="RadioGroup1_1" class="radio" /></td>   
								</tr>    
                                <? } ?>   
								<tr>
									<td colspan="2">&nbsp;</td>
							...
							<?php
								...
								if ($_GET["done"] == 3)
								{
									echo "<div align='left'><font color='#FF0000'>Record(s) has been archived successfully.<br></font></div>";
								}
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Record [".$currencyshortname."] is existed in the system. Please enter another Record.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT c._id, c._currencyshortname,c._sign, c._subdate,  c._status, u._Fullname as _subby, ss._statusname FROM ".$tbname."_currencies c 
							INNER JOIN ".$tbname."_systemstatus ss ON ss._id = c._status  
							INNER JOIN ".$tbname."_user u ON u._ID = c._subby  
							";							
							if ($Keywords != "") $str1 = $str1 . " AND c._currencyshortname LIKE '%".replaceSpecialChar($Keywords)."%' ";
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY c._status , c._currencyshortname ASC LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Currency List</b></td>
								</tr>
                               	<form name="search_form" action="currencies.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="text" tabindex="" title="Currency" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
                                <tr>
                                  <td></td>
                        ...
                        elseif (trim($sortArrange) == "ASC")
							...
							{
								?>
                                    <a href="currencies.php" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="1" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._currencyshortname&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Currency</a></td>
                                               												<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._sign&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Short Code</a></td>
                                                
                                                <td class="gridheader datecolumn" align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._subby&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Submitted By</a></td>
												<td class="gridheader datecolumn" align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._subdate&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Submitted Date</a></td>
                                                <td class="gridheader" align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._status&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">System Status</a></td>
