								...
								<tr>
								  <td align="left" class="TitleStyle"><b>Traning Venue</b></td>
                               		...
                               		<?php
                                            ...
                                            $str1 = "SELECT v.*,vo._FullName,vc._VenueCatName,vt._VenueTypeName,m._MembershipType	 FROM ".$tbname."_venues v  LEFT JOIN  ".$tbname."_venueowners vo ON v._OwnerID = vo._ID 
											LEFT JOIN  ".$tbname."_venuecat vc ON v._VenueCat = vc._ID  
											LEFT JOIN  ".$tbname."_venuetype vt ON v._VenueType = vt._ID 
											LEFT JOIN  ".$tbname."_membershipstatus m ON v._membershipStatus = m._ID WHERE v._ID > 0";

                                            if($Keywords != "Enter Any Keywords" && $Keywords != "")
                                            {
												$str1 = $str1 . " AND (vo._FullName LIKE '%".replaceSpecialChar($Keywords)."%' ";
                                            ...
                                            else if($owner != "Enter Owner Name" && $owner != "")
                                            {
                                                ...
                                                $str1 = $str1 . " AND (vo._FullName LIKE '%".replaceSpecialChar($owner)."%' )";
											}
											...
											if($status != "")
											{
												$str1 = $str1 . " AND v._Status = ". $status ."";						
											}
											$str1 .= " GROUP BY v._ID ";
                                            ...
                                            <td colspan="2">
                                                ...
                                                <table cellspacing="0" cellpadding="3" width="100%" border="0" class="grid" >
                                                    <tr>
                                                        ...
                                                        <td class="gridheader" width="120" align="center">
                                                    ...
                                                    while($rs = mysql_fetch_assoc($result))
                                                    {
                                                        ...
                                                        <tr class="clickableRow" href="trainingspace.php?PageNo=<?=encrypt($PageNo,$Encrypt)?>&amp;id=<?=encrypt($rs["_ID"],$Encrypt)?>&amp;e_action=<?=encrypt("edit",$Encrypt)?>" style="cursor:pointer" >
                                                            ...
                                                            <td id="Row4ID<?=$i?>" class="<?php echo $Rowcolor; ?>" width="100"><?=replaceSpecialCharBack($rs["_VenueName"])?></td>
                                                            <td id="Row5ID<?=$i?>" class="<?php echo $Rowcolor; ?>" align="left" ><?php echo ($rs['_FullName']) ?></td>
                                                            <td id="Row8ID<?=$i?>" class="<?php echo $Rowcolor; ?>" align="center" ><?=replaceSpecialCharBack($rs["_VenueCatName"])?></td>
                                                            <td id="Row9ID<?=$i?>" class="<?php echo $Rowcolor; ?>" align="center" ><?=replaceSpecialCharBack($rs["_VenueTypeName"])?></td>
