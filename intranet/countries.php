...
if(count($Operations)== 0)
{
 echo "<script language='javascript'>history.back();</script>";
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_countries WHERE _id = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$CountryName = $rs["_countryname"];		
			$status = $rs["_status"];			
			$btnSubmit = "Update";
		}
	}
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.CountryName.value == 0)
                errormsg += "Please fill in 'Country'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "countries_action.php";
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings &gt; System &gt; Countries</b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							...
							<form name="FormName" action="countries_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?=$id?>" />
							<input type="hidden" name="e_action" value="<?=($e_action == "" ? "Add" : "Edit")?>" />
							<input type="hidden" name="PageNo" value="<?=$_GET['PageNo']?>" />
							<table cellpadding="1" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b>Countries</b></td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
									<td width="120">Country </td>
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="CountryName" value="<?=$CountryName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>
                                <? if ($e_action=="edit") { ?>								
								<tr>
									<td nowrap>&nbsp;Status&nbsp;</td>
							...
							<?php
								...
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Record [".$CountryName."] is existed in the system. Please enter another Record.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT c._id, c._countryname, c._subdate,  c._status, ss._statusname,_Fullname FROM ".$tbname."_countries c 
							INNER JOIN ".$tbname."_systemstatus ss ON ss._id = c._status		
							INNER JOIN ".$tbname."_user u ON u._ID = c._subby ";					
							if ($Keywords != "") $str1 = $str1 . " AND c._countryname LIKE '%".replaceSpecialChar($Keywords)."%' ";
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY c._status , c._countryname ASC LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Country List</b></td>
								</tr>
                               	<form name="search_form" action="countries.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="text" tabindex="" title="Country" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
                                <tr>
                                  <td></td>
                        ...
                        elseif (trim($sortArrange) == "ASC")
							...
							{
								?>
                                    <a href="countries.php" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="1" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=c._countryname&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Country</a></td>
