<?php
	...
	if($id != "" && $e_action == 'edit')
	{ 
		$str = "SELECT * FROM ".$tbname."_trainingprogramcat WHERE _ID = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$ProgramCategoryName = $rs["_ProgramCatName"];	
			$status = $rs["_Status"];	
			$CommunityID = $rs["_CommunityID"];	
...
<link rel="stylesheet" type="text/css" href="../css/admin.css" />			
	...
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.ProgramCategoryName.value == 0)
                errormsg += "Please fill in 'Add Programs Category'.\n";
        ...
        function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "programcategories_action.php?e_action=curdelete";
			...
			<div class="maintable">
				<table width="100%"  border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
                        ...
                        <tr>
                          <td align="left" class="TitleStyle"><b>Settings > Programs Category </b></td>
                        </tr>
                        <tr><td height=""></td></tr>
                    </table>
                    <form name="FormName" action="programcategories_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
                    ...
                    <tr><td height="25"></td></tr>    
                     <tr>
                     <td width="130" valign="middle">Add Program Category</td>
                     <td width="10" valign="middle">&nbsp;:&nbsp;</td>
                     <td valign="middle"><input type="text" tabindex="" name="ProgramCategoryName" value="<?=$ProgramCategoryName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
                     </tr>
 		                               
                         <tr><td height="15"></td></tr>
                        <tr>
                        <td width="130" valign="middle">Community</td>
                        <td width="10" valign="middle">&nbsp;:&nbsp;</td>
                        <td  valign="middle">
                        <? $str = "SELECT _ID,_CommunityName FROM ".$tbname."_communities ORDER BY _ID";
                            $rst = mysql_query($str, $connect) or die(mysql_error());?>
                          <select  tabindex="" name="communityID" id="communityID" class="dropdown1  chosen-select">
                        ...
                        if(mysql_num_rows($rst) > 0)
                        {
                            ...
                            {
                            ?>
                            <option value="<?php echo $rs["_ID"]; ?>" <?php if($rs["_ID"] == $CommunityID) {?> selected <?php } ?>>&nbsp;<?php echo $rs["_CommunityName"]; ?>&nbsp;</option>
                            <?php
                            }
                         }
                        ?>
						...
						<tr >
										...
										{ ?> 
											 ...
											 <a href="javascript:void(0);" onClick="if(confirm('Are you sure you want to delete this?')) { window.open('programcategories_action.php?e_action=deletefile&removID=<?php echo $id;?>&Types=I','personalize','toolbar=no,location=no,directories=no,status=no,menubar=no'); return true; }" onMouseOver="write_it('Delete File');return true;" class="link1">[<img src="../images/delfilepic.gif" border="0" alt="Delete File"> Delete File]</a>
										<? }else {?>
                        ...
                        <? } ?>  
							...
							<?php
								if ($_GET["done"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>ProgramCategory has been added successfully.<br></font></div>";
								}
								if ($_GET["done"] == 2)
								{
									echo "<div align='left'><font color='#FF0000'>ProgramCategory has been edited successfully.<br></font></div>";
								}
								if ($_GET["done"] == 3)
								{
									echo "<div align='left'><font color='#FF0000'>ProgramCategory has been deleted successfully.<br></font></div>";
								}
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>ProgramCategory [".$ProgramCategoryName."] is existed in the system. Please enter another Category.<br></font></div>";
								}
							     ?>
                                <?php
							...
							$i = 1;							
							$str1 = "SELECT pc._ID,IF(pc._Status = 1, 'Live', 'Archived') as _Status,_ProgramCatName,pc._CreatedDateTime,com._CommunityName,usr._Fullname as _subby 
							FROM ".$tbname."_trainingprogramcat  pc
							left join  ".$tbname."_user usr on (pc._SubmittedBy = usr._ID)	 
							left join  ".$tbname."_communities com on (pc._CommunityID = com._ID)
							WHERE pc._Status <> 0 ";		

							
							if ($Keywords != "") $str1 = $str1 . " AND _ProgramCatName LIKE '%".replaceSpecialChar($Keywords)."%' ";

							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . "ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . "ORDER BY pc._Status,_ProgramCatName LIMIT $StartRow,$PageSize ";
                            ...
                            <tr>
                                <td colspan="2"><b>Program Category List</b></td>
                            </tr>
                                <form name="search_form" action="progcategories.php" method="get" onsubmit="clearDefault()">
                            <tr> 
                        ...
                        {
                            ?>
                                    <a href="progcategories.php?type=<?=$type?>" class="TitleLink">Add New</a>
                        ...
                        ?>
                            ...
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <td class="gridheader">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_ProgramCatName&amp;sortArrange=<?=$sortArrange?>" class="link1">Category Name</a></td>
                                    <td class="gridheader">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_CommunityName&amp;sortArrange=<?=$sortArrange?>" class="link1">Community Name</a></td>
                                    ...
                                    <td class="gridheader datecolumn" align="center">&nbsp;&nbsp;<a href="?type=<?=$type?>&PageNo=<?=$PageNo?>&amp;sortBy=_CreatedDateTime&amp;sortArrange=<?=$sortArrange?>" class="link1">Submitted Date</a></td>                                  
                                ...
                                if($RecordCount!="")
                                {
                                    ...
                                    while($rs = mysql_fetch_assoc($result))
                                    {
                                        ...
                                        $createdDate=$rs['_CreatedDateTime'];
                                        ...
                                        <tr >
                                            ...
                                            <?php													
                                            echo $rs["_ProgramCatName"]
                                            ?>&nbsp;
                                            </td> 

                                            <td id="Row3ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="left" >&nbsp;
                                            <?php													
                                            echo $rs["_CommunityName"]
                                            ?>&nbsp;
                                            </td> 
