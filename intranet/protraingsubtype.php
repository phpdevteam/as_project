<?php
 ...
 include("fckeditor/fckeditor.php");
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_trainingsubtype WHERE _ID = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$SubTypeName = $rs["_SubTypeName"];	
			$TypeID = $rs["_TypeID"];	
			$nosession = $rs["_NoSession"];	
			$status = $rs["_Status"];		
			$btnSubmit = "Update";
		}
	}
	...
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
            ...
            if (document.FormName.TypeID.value == 0)
                errormsg += "Please Select 'Program Type'.\n";
				
		    if (document.FormName.SubTypeName.value == 0)
                errormsg += "Please Fill in 'Programs Sub Type'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "protraingsubtype_action.php?e_action=curdelete";
					...
					<tr>
						...
						-->
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings &gt; System &gt; Programs Sub Type  </b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							<form name="FormName" action="protraingsubtype_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b>Programs Sub Type</b></td>
								</tr>
								...
								<tr>
									...
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="SubTypeName" value="<?=$SubTypeName?>" class="txtbox1" style="width:220" /> <span class="detail_red">*</span></td>
								</tr>	
								<tr><td height="5"></td></tr>
                                	
								<tr>
									<td width="120">No Of Session </td>
									<td width="10">&nbsp;:&nbsp;</td>
									<td><input type="text" tabindex="" name="nosession" value="<?=$nosession?>" class="txtbox1" style="width:220" /> </td>
								</tr>	
							...
							<?php
								...
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Province [".$SubTypeName."] is existed in the system. Please enter another Province.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT st._ID,st._SubTypeName,st._NoSession,IF(st._Status = 1, 'Live', 'Archived') as _status,t._TypeName,st._CreatedDateTime FROM ".$tbname."_trainingsubtype st 
							inner JOIN ".$tbname."_trainingtype t ON t._ID = st._TypeID where st._Status > 0 ";							
							
							if ($Keywords != "") $str1 = $str1 . " AND ( st._SubTypeName LIKE '%".replaceSpecialChar($Keywords)."%' or t._TypeName like '%".replaceSpecialChar($Keywords)."%' ) ";
							
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY st._TypeID,st._SubTypeName LIMIT $StartRow,$PageSize ";
                            ...
                            <tr><td height="10"></td></tr>
								<tr>
									<td colspan="2"><b>Programs Sub Type List</b></td>
								</tr>
								<form name="search_form" action="protraingsubtype.php" method="get" onsubmit="clearDefault()"> <input type="hidden" name="cid" value="<?=$cid?>" />
                                <tr> 
                                  <td><input type="text" tabindex="" title="Programs Sub Type" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" />&nbsp;&nbsp;<input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
							...
							if(in_array("Add",$Operations))
							{
								?>
                                    ...
                                    <a href="protraingsubtype.php?cid=<?=$cid?>" class="TitleLink">Add New</a>
                            	...
                            	<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_TypeName&amp;sortArrange=<?=$sortArrange?>" class="link1">Programs Type</a></td>
                            
                            					<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_SubTypeName&amp;sortArrange=<?=$sortArrange?>" class="link1">Programs Sub Type</a></td>

												<td class="gridheader">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_NoSession&amp;sortArrange=<?=$sortArrange?>" class="link1">No Of Session</a></td>
												...
												<td class="gridheader datecolumn" align="center">&nbsp;&nbsp;<a href="?PageNo=<?=$PageNo?>&amp;sortBy=_CreatedDateTime&amp;sortArrange=<?=$sortArrange?>" class="link1">Submitted Date</a></td>
