...
$Operations = GetAccessRights(185);
	...
	if($id != "" && $e_action == 'edit')
	{
		$str = "SELECT * FROM ".$tbname."_gstmaster WHERE _id = '".$id."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rs = mysql_fetch_assoc($rst);
			$ratename = $rs["_ratename"];	
			$rate = $rs["_rate"];		
	...
	if($_GET['error'] == 1)
	{
		$str = "SELECT * FROM ".$tbname."_tax WHERE _id = '". $_GET["ratename"]."' ";
		$rst = mysql_query("set names 'utf8';");	
		$rst = mysql_query($str, $connect) or die(mysql_error());
		if(mysql_num_rows($rst) > 0)
		{
			$rrs = mysql_fetch_assoc($rst);
			$myratename = $rrs["_TaxName"];
   ...
   <script type="text/javascript" src="../js/functions.js"></script>
	<script type="text/javascript" language="javascript">
		...
		function validateForm()
		{
			var errormsg;
            errormsg = "";					
            if (document.FormName.ratename.value == 0)
                errormsg += "Please select 'Tax Name'.\n";
		...
		function validateForm3()
		{
			...
			else
			{
				if(confirm('Are you sure you want to archive the selected Record(s)?') == true)
				{
					...
					document.forms.FormName2.action = "taxrates_action.php";
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
								  <td align="left" class="TitleStyle"><b>Settings - System - Tax Rates</b></td>
								</tr>
								<tr><td height=""></td></tr>
							</table>
							<form name="FormName" action="taxrates_action.php" method="post" onsubmit="return validateForm();" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?=$id?>" />
							<input type="hidden" name="e_action" value="<?=($e_action == "" ? "Add" : "Edit")?>" />
							<input type="hidden" name="PageNo" value="<?=$_GET['PageNo']?>" />
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td style="padding-top:5px;" colspan="3"><b><?=($e_action == "" ? "Add" : "Edit")?> Tax Rate</b>
                                    <br />
                                    (Note: Only 1 Type of Tax Rate Allowed At Each Time)
                                    </td>
								</tr>
								<tr><td height="10"></td></tr>
								<tr>
									<td width="150" valign="middle">Tax Name </td>
									<td width="10"  valign="middle">&nbsp;:&nbsp;</td>
									<td  valign="middle">
                                    <select  tabindex="" name="ratename" id="ratename" class="txtbox1 chosen-select" style="width:220">
										<option value="">--select--</option>
										<?php
											$sql = "SELECT * FROM ".$tbname."_tax  Order By _TaxName";
											$res = mysql_query($sql) or die(mysql_error());
											if(mysql_num_rows($res) > 0){
												while($rec = mysql_fetch_array($res)){
													?><option <?php if($rec['_ID'] == $ratename){ echo 'selected="selected"'; } ?> value="<?php echo $rec['_ID']; ?>"><?php echo $rec['_TaxName']; ?></option><?php
												}
											}
										?>
										</select>
                                ...
                                <tr>
									<td width="120"  valign="middle">Rate</td>
									<td width="10"  valign="middle">&nbsp;:&nbsp;</td>
									<td  valign="middle"><input type="text" tabindex="" name="rate" value="<?=$rate?>" class="txtbox1" style="width:220" onkeydown="return currenciesonly(event);"/>
								    <span class="detail_red">*</span></td>
								</tr>
                                ...
                                <tr>
                                	...
                                	<td  valign="middle">
										...
										<input type="text" tabindex="" id="startdate"  name="startdate" value="<?php echo $startdate; ?>" style="width:100px;" class="datepicker txtbox1" readonly="readonly" />&nbsp;/ &nbsp;<input type="text" tabindex="" id="enddate" name="enddate" value="<?php echo $enddate; ?>" style="width:100px;" class="datepicker txtbox1" readonly="readonly" />
									</td>
                                </tr>
                                ...
                                <? if ($e_action=="edit") { ?>								
								<tr>
									<td width="150px"  valign="middle">&nbsp;Status&nbsp;</td>
									<td  valign="middle">&nbsp;:&nbsp;</td>
									<?php
									$sel_status_y = "";
									$sel_status_n = "";
							...
							<?php
								...
								if ($_GET["error"] == 1)
								{
									echo "<div align='left'><font color='#FF0000'>Record [".$myratename."] is existed in the system. Please enter another Record.<br></font></div>";
								}
							?>
							<?php
							...
							$i = 1;							
							$str1 = "SELECT g._id, tax._TaxName as _TaxName, g._rate, g._startdate, g._enddate, g._subdate,  g._status, u._Fullname as _subby, ss._statusname 
							FROM ".$tbname."_gstmaster g
							INNER JOIN ".$tbname."_tax tax ON g._ratename = tax._id  
							INNER JOIN ".$tbname."_systemstatus ss ON ss._id = g._status  
							INNER JOIN ".$tbname."_user u ON u._ID = g._subby  
							";							
							if ($Keywords != "") $str1 = $str1 . " AND g._TaxName LIKE '%".replaceSpecialChar($Keywords)."%' ";
							if (trim($sortBy) != "" && trim($sortArrange) != "")
								$str2 = $str1 . " ORDER BY ".trim($sortBy)." ".trim($sortArrange)." LIMIT $StartRow,$PageSize ";
							else
								$str2 = $str1 . " ORDER BY g._status,_TaxName,g._startdate DESC LIMIT $StartRow,$PageSize ";
							...
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr>
									<td colspan="2"><b>Tax Rate List</b></td>
								</tr>
                               	<form name="search_form" action="taxrates.php" method="get" onsubmit="clearDefault()">
                                <tr> 
                                  <td><input type="text" tabindex="" title="Rate Name" name="Keywords" id="Keywords" value="<?=$Keywords?>" class="defaultText" /><input type="submit" name="btnSearch" class="button1" value="Search" /></td>                                  
                                </tr>
                                </form>
                                <tr>
                                  <td></td>
                        ...
                        elseif (trim($sortArrange) == "ASC")
							...
							{
								?>
                                    <a href="taxrates.php" class="TitleLink">Add New</a>
									...
									<td colspan="2">
										<table cellspacing="0" cellpadding="0" width="100%" border="0" class="grid">
											<tr>
												...
												<td class="gridheader" > <a href="?PageNo=<?=$PageNo?>&amp;sortBy=tax._TaxName&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Rate Name</a></td>
                                                <td class="gridheader"  align="right"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._rate&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Rate </a></td>
                                                <td class="gridheader"  align="center"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._startdate&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Start Date</a></td>
                                                 <td class="gridheader"  align="center"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._enddate&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">End Date</a></td>
												 
												  <td class="gridheader datecolumn" align="center"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._subby&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Submitted By</a></td>
												 
                                                <td class="gridheader datecolumn" align="center"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._subdate&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">Submitted Date</a></td>
                                               
                                                <td class="gridheader" align="center"><a href="?PageNo=<?=$PageNo?>&amp;sortBy=g._status&amp;sortArrange=<?=$sortArrange?>&amp;Keywords=<?=$Keywords?>" class="link1">System Status</a></td>
												...
												while($rs = mysql_fetch_assoc($result))
												{
													...
													<tr >
													  ...
													  <input name="CustCheckbox<?php echo $i; ?>" type="checkbox"   tabindex="" value="<?php echo $rs["_id"]; ?>" onclick="setActivities(this.form.CustCheckbox<?php echo $i; ?>, <?php echo $i; ?>,'<?=$Rowcolor?>');" />
                                                        ...
                                                        <td id="Row4ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="right" >&nbsp;
														<?php													
														echo $rs["_rate"]
														?>&nbsp;
														</td>
                                                        <td id="Row5ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_startdate"]!=""?date(DEFAULT_DATEFORMAT,strtotime(replaceSpecialCharBack($rs["_startdate"]))):"" ?>&nbsp;</td>
                                                        <td id="Row6ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_enddate"]!=""?date(DEFAULT_DATEFORMAT,strtotime(replaceSpecialCharBack($rs["_enddate"]))):"" ?>&nbsp;</td>   
                                                      
                                                        <td id="Row7ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_subby"]; ?>&nbsp;</td>
                                                        
														  <td id="Row8ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_subdate"]!=""?date(DEFAULT_DATEFORMAT,strtotime(replaceSpecialCharBack($rs["_subdate"]))):"" ?>&nbsp;</td>
														<td id="Row9ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;<?php echo $rs["_statusname"]; ?>&nbsp;</td>                                                
														<td id="Row10ID<?=$i?>" class="<?php echo $Rowcolor; ?>" valign="top" align="center">&nbsp;
