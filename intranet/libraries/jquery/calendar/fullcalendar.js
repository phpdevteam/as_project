 ...
 *
 * Date: Tue Jul 26 00:36:10 2011 +0100
 *
 */
...
function Calendar(element, options, eventSources) {
	...
	function changeView(newViewName) {
		$.session("defaultview",newViewName);
...
function ResourceView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		s =
			...
			"<thead>" +
			"<tr>" +
			"<th class='fc-agenda-axis shClass " + headerClass + "' >&nbsp;</th>";
		for (i=0; i<colCnt; i++) {
			s +=
				"<th class='myheader sanjeev01 fc- fc-col" + i + ' ' + headerClass + "' id='teacher" + i + "' />"; // fc- needed for setDayID
		}
		s +=
			"<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>" +
			"</tr>" +
		...
		slotScroller =
			$("<div id='isSlotController' style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto'/>")
				.appendTo(slotLayer);
		...
		eventElement.draggable({
			...
			stop: function(ev, ui) {
				...
				if (cell && (columnDelta || minuteDelta || allDay)) {
					...
					$.session("changeid",event.resource.id);
